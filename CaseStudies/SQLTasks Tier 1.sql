...
FROM 
	Members AS m1
INNER JOIN 
	Members AS m2  -- it's a self join!
	ON m1.recommendedby = m2.memid
WHERE 
	m1.recommendedby != 0 -- to get rid of it saying a guest recommended them
ORDER BY
	member_lastname,
    member_firstname,
...
SELECT
	f.name,
    COUNT(b.slots) AS facility_usage
FROM 
	Facilities AS f
INNER JOIN 
	Bookings AS b
    ON f.facid = b.facid
WHERE
	b.memid > 0 -- doesn't include guests
GROUP BY 
	f.name
